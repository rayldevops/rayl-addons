...
class SaasClient(models.Model):
    ...
    def login_to_client_instance(self):
        for obj in self:
            ...
            _logger.info("Host Server : .................. %s", host_server)
            _logger.info("DB Server : .................. %s", db_server)
            ...
            if response:
                ...
                _logger.info("Login : .................. %s", response[0][0])
                _logger.info("Password : .................. %s", response[0][1])
                login_url = "{}/saas/login?db={}&login={}&passwd={}".format(obj.client_url, obj.database_name, login, password)
                _logger.info("Password : .................. %s", login_url)
                return {
    ...
    def synch_rayl_community_user(self):
        import requests
        import json
        import re
        for obj in self:
            ...
            if response:
                ...
                rayl_community_url = setting_params.get_param(
                    'rayl_community.rayl_community_user_exists_url')
                rayl_community_update_url = setting_params.get_param(
                    'rayl_community.rayl_community_user_update_url')
                rayl_community_username = setting_params.get_param(
                    'rayl_community.rayl_community_admin_username')
                rayl_community_password = setting_params.get_param(
                    'rayl_community.rayl_community_admin_password')

                current_user_email_id = obj.partner_id.email
                ...
                if not re.match(regex, current_user_email_id):
                    ...
                    apptive_share_username = obj.database_name
                    apptive_db_name = obj.database_name
                    apptive_email = current_user_email_id
                    apptive_name = partner_name.replace(" ", "-")
                    ...
                    if req:
                        ...
                        _logger.info(user_update_details)
                        ...
                        _logger.info(user_insert_details)
                    ...
                    if req_up:
                        ...
                        if req:
                            _logger.info("User Update in Community with Email : " + str(
                                apptive_email) + " And Password : P@ssword")
                            raise UserError("User Update in Community with Email : " + str(
                                apptive_email) + " And Password : P@ssword")
                        else:
                            _logger.info("User Created in Community with username : " + str(
                                apptive_email) + " And Password : P@ssword")
                            raise UserError("User Created in Community with Email : " + str(
                                apptive_email) + " And Password : P@ssword")
                    else:
                        res_up = json.loads(req_up.content.decode("utf-8"))
                        _logger.info(res_up)
