...
#        return used_ports
    ...
    def find_me_an_available_port_within(self,a,b,c = None):
        ...
        # cmd = "python3 /opt/find_me_a_port.py %r %r %r" % (a, b, c)
    ...
    def run_odoo(self,name, db):
        ...
        try:
            ...
            _logger.info("Port CHeck %r"%port)
            if port == False:
                return False
    ...
    def login_remote(self):
        try:
            ...
            k = paramiko.RSAKey.from_private_key_file("/opt/RAYL_RSA")
            ssh_obj.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_obj.connect(hostname=self.remote_host, username=self.remote_user, pkey = k,port=self.remote_port)
            return ssh_obj
        except Exception as e:
...
class nginx_vhost:
    ...
    def login_remote(self,remote_host,remote_user,remote_passwd):
        try:
            ...
            k = paramiko.RSAKey.from_private_key_file("/opt/RAYL_RSA")
            ssh_obj.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_obj.connect(hostname=remote_host, username=remote_user, pkey = k)
            return ssh_obj
        except Exception as e:
            _logger.info("Couldn't connect remote")
            return False
...
def main(context=None):
    ...
    try:
        ...
        _logger.info(OdooObject.execute_on_remote_shell(ssh_obj,"chown -R odoo:odoo %s"%dest))    #########----->>> do we really need it
    except OSError as e:
