...
def ishostaccessible(details):
    ...
    try:
        ...
        ssh_obj.connect(hostname = details['host'], username = details['user'], pkey = k, port = details['port'])
        return ssh_obj
...
def isdbaccessible(host_server,db_server,config_path=None):
    ...
    try:
        ...
        k = paramiko.RSAKey.from_private_key_file("/opt/RAYL_RSA")
        ssh_obj.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_obj.connect(hostname = host_server['host'], username = host_server['user'],pkey = k, port = host_server['port'])
